...
BG_APP = "#0f172a"        # Fundo principal
BG_CARD = "#1e293b"       # Fundo dos cards
BG_INPUT = "#334155"      # Fundo de inputs e listboxes
FG_TEXT = "#f8fafc"       # Texto principal (Branco/Gelo)
FG_MUTED = "#94a3b8"      # Texto secundário (Cinza)
ACCENT_BLUE = "#3b82f6"   # Botões primários
ACCENT_GREEN = "#10b981"  # Botão de Calcular
ACCENT_RED = "#ef4444"    # Botões de perigo/remover
...
class CGFApp(tk.Tk):
    ...
    def _config_style(self):
        ...
        style.configure("Result.TLabel", font=("Segoe UI", 16, "bold"), foreground=ACCENT_GREEN, background=BG_CARD)
        ...
        # Notebook (Abas) modernizadas
        style.configure("TNotebook", background=BG_CARD, borderwidth=0, tabmargins=[0, 0, 0, 0])
        style.configure("TNotebook.Tab", font=("Segoe UI", 10, "bold"), padding=[15, 8], background=BG_INPUT, foreground=FG_MUTED, borderwidth=0)
    ...
    def _build_ui(self):
        ...
        ttk.Label(header, text="Fluxo: Faturada - Canceladas - Devoluções - Consumo próprio.", style="SubHeading.TLabel").pack(anchor="w", pady=(2, 0))
        ...
        # Renderização dos Cards (Substituindo LabelFrames velhos)
    ...
    def _create_card(self, parent):
        """Cria um frame liso que atua como um card moderno"""
    ...
    def _build_files_card(self, parent):
        ...
        self.files_listbox = tk.Listbox(
            content, bg=BG_INPUT, fg=FG_TEXT, selectbackground=ACCENT_BLUE,
            selectforeground=FG_TEXT, highlightthickness=0, borderwidth=0,
    ...
    def _add_form_row(self, parent, label_text, variable, row_idx):
        ...
        ttk.Entry(parent, textvariable=variable, font=("Segoe UI", 10)).grid(row=row_idx, column=1, sticky="ew", pady=5, padx=(10,0))

    def _build_extra_cols_section(self, parent, start_row, nome, cmd_add, cmd_rem):
        ttk.Label(parent, text="Colunas extras de rastreio (Opcional):", style="CardText.TLabel").grid(row=start_row, column=0, columnspan=2, sticky="w", pady=(0, 5))
        ...
        lst = tk.Listbox(
            parent, bg=BG_INPUT, fg=FG_TEXT, selectbackground=ACCENT_RED,
            highlightthickness=0, borderwidth=0, font=("Segoe UI", 9), height=3
        )
        lst.grid(row=start_row+2, column=0, columnspan=2, sticky="nsew", pady=10)

        ttk.Button(parent, text="Remover Selecionada", style="Danger.TButton", command=cmd_rem).grid(row=start_row+3, column=0, columnspan=2, sticky="ew")
        return entry, lst
    ...
    def _build_result_card(self, parent):
        ...
        ttk.Button(header_box, text="▶ INICIAR CÁLCULO", style="Accent.TButton", command=self.calculate_total).pack(side="right")

        res_box = tk.Frame(content, bg=BG_INPUT, padx=15, pady=15)
        res_box.pack(fill="x", pady=(0, 15))
        self.result_label = ttk.Label(res_box, text="Volume Final CGF: ---", style="Result.TLabel", background=BG_INPUT)
        ...
        self.log_text = tk.Text(
            ...
            highlightthickness=0, borderwidth=0, font=("Consolas", 10), wrap="word"
        )
    ...
    # -----------------------------------------
    # Lógica de Leitura e Cálculo (Intacta)
    # -----------------------------------------
    ...
    def calculate_total(self):
        ...
        fat_vol_col, fat_cons_col = self.col_fat_volume.get().strip(), self.col_fat_consumo.get().strip()
        fat_cons_val, fat_cfop_col = self.val_fat_consumo.get().strip(), self.col_fat_cfop.get().strip()
        canc_vol_col, dev_vol_col = self.col_canc_volume.get().strip(), self.col_dev_volume.get().strip()
        ...
        for path in self.selected_files:
            ...
            if "faturada" in nome_low and "complementar" in nome_low:
                ...
                if fat_cons_col and fat_cons_col in df.columns and fat_cons_val:
                    ...
                    df_cons, df_sem_cons = df[mask_cons].copy(), df[~mask_cons].copy()
                    ...
                    vol_fat, vol_cons = df_sem_cons[fat_vol_col].sum(), df_cons[fat_vol_col].sum()
